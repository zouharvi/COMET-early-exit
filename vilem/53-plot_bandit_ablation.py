...
with open("../computed/bandit_start_ablation_1000_norm_ucb05sample.pkl", "rb") as f:
    data_sample = pickle.load(f)
with open("../computed/bandit_start_ablation_1000_norm_ucb05beam.pkl", "rb") as f:
    data_beam = pickle.load(f)

METHOD_TO_CONFIG = {
    'bandit (start=0)': dict(name="Bandit ($\\geq$layer 1)", color=utils_figs.COLORS[0], alpha=1),
    'bandit (start=4)': dict(name="Bandit ($\\geq$layer 5)", color=utils_figs.COLORS[0], alpha=0.5),
    'bandit (start=12)': dict(name="Bandit ($\\geq$layer 13)", color=utils_figs.COLORS[0], alpha=0.2),
...
for name, data in [("beam", data_beam), ("sample", data_sample)]:
    for mode in ["avg_score", "win_rate"]:
        ...
        plt.savefig(f"../figures/53-bandit_ablation_{mode}_{name}.pdf")
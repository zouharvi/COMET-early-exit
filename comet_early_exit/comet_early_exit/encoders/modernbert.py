...
ModernBERT Encoder
==============
    Pretrained ModernBERT encoder from Hugging Face.
"""
...
class ModernBERTEncoder(Encoder):
    ...
    def __init__(
        ...
        if load_pretrained_weights:
            ...
            self.model = ModernBertModel(
                ModernBertConfig.from_pretrained(
    ...
    def from_pretrained(
        ...
        """
        return ModernBERTEncoder(pretrained_model, load_pretrained_weights, local_files_only)
    ...
    def layerwise_lr(self, lr: float, decay: float):
        ...
        for i in range(2, self.num_layers):
            opt_parameters.append(
                {
                    "params": self.model.layers[-i].parameters(),
                    "lr": lr * decay ** (i - 1),
                }
            )
        # Embedding Layer

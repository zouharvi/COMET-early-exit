...
class EarlyExitRegression(RegressionMetric):
    ...
    def __init__(
        ...
        if self.hparams.layer == "mix":
            # WARN: this is sequential just so we can store a list in there but don't use it sequentially!
            # instead, index into it like self.layrwise_attention[0]
            self.layerwise_attention = torch.nn.Sequential(*[
                LayerwiseAttention(
                    layer_transformation=layer_transformation,
                    num_layers=i,
                    dropout=self.hparams.dropout,
    ...
    def predict(
        ...
        See earlyexitconf.py for a more complete version.
        """

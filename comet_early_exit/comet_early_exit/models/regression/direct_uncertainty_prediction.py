...
from comet_early_exit.modules import FeedForward, Bottleneck


class DirectUncertaintyPrediction(RegressionMetric):
    """DirectUncertaintyPrediction:
    ...
    def __init__(
        ...
        hidden_sizes_bottleneck: List[int] = [256],
        feature_size: int = 1,
        ...
        super(RegressionMetric, self).__init__(
            ...
            class_identifier="direct_uncertainty_prediction",
    ...
    def prepare_sample(
        ...
        # import sys
        # print("A", type(inputs["predicted"]), file=sys.stderr)
        # print("B", len(inputs["predicted"]), file=sys.stderr)
        # print("C", inputs["predicted"][0], file=sys.stderr)
        # print("D", type(inputs["predicted"][0]), file=sys.stderr)
        # print(file=sys.stderr)

        inputs["predicted"] = [float(x) for x in inputs["predicted"]]
        custom_features = torch.tensor(inputs["predicted"]).unsqueeze(1)
        model_inputs = {**src_inputs, **mt_inputs, "custom_features": custom_features}
    ...
    def forward(
        ...
        **kwargs
        ...
        return Prediction(score=self.estimator(seq_feats).view(-1))
    ...
    def read_training_data(self, path: str) -> List[dict]:
        ...
        """
        df = pd.read_csv(path)
        df = df[["src", "mt", "score", "predicted"]]
        df["src"] = df["src"].astype(str)
        df["mt"] = df["mt"].astype(str)
        df["score"] = df["score"].astype("float16")
        df["predicted"] = df["predicted"].astype("float16")
    ...
    def read_validation_data(self, path: str) -> List[dict]:
        ...
        """
        df = pd.read_csv(path)
        columns = ["src", "mt", "score", "predicted"]

...
r"""
EarlyExitConfRegression
========================
    Early exit conf referenceless regression Metric that learns to predict a quality assessment by
...
class EarlyExitConfMultiRegressionExtra(RegressionMetric):
    """EarlyExitConfMultiRegression:
    ...
    def __init__(
        ...
        confidence_target: Literal["last", "human"] = None
    ) -> None:
        ...
        self.estimator = FeedForward(
            # two extra for current layer index and current layer traineable
            in_dim=self.encoder.output_units * 4 + 2,
        ...
        self.layer_index_param = torch.nn.Parameter(torch.zeros(self.encoder.num_layers, dtype=torch.float32), requires_grad=True)
        self.layer_index_fixed = torch.tensor([i for i in range(self.encoder.num_layers)], dtype=torch.float32)

        self.confidence_target = confidence_target
        assert confidence_target is not None, "Confidence target must be set for EarlyExitConfRegression"

        self.loss_score = torch.nn.MSELoss()
        self.loss_confidence = torch.nn.MSELoss()

        if self.hparams.layer == "mix":
            self.layerwise_attention = torch.nn.ModuleList([
                LayerwiseAttention(
                    layer_transformation=layer_transformation,
                    num_layers=i,
                    dropout=self.hparams.dropout,
                ...
                for i in range(1, self.encoder.num_layers+1)
            ])
        else:
    ...
    def forward(
        ...
        **kwargs
        ...
        # move self.layer_index_param and self.layer_index_fixed to the same device as embedded_sequences
        self.layer_index_param = self.layer_index_param.to(embedded_sequences.device)
        self.layer_index_fixed = self.layer_index_fixed.to(embedded_sequences.device)

        batch_size = embedded_sequences.shape[1]
        embedded_sequences = torch.cat([
            embedded_sequences,
            self.layer_index_param.unsqueeze(1).expand(-1, batch_size).unsqueeze(-1),
            self.layer_index_fixed.unsqueeze(1).expand(-1, batch_size).unsqueeze(-1)
        ], dim=-1)
    ...
    def compute_sentence_embedding(
        ...
        return torch.stack([x[:,0,:] for x in embeddings])

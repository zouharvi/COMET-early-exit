...
class LayerwiseAttention(torch.nn.Module):
    ...
    def forward(
        ...
        if override_num_layers is None and len(tensors) != self.num_layers:
            raise Exception(
        ...
        if override_num_layers is not None:
            scalar_parameters = scalar_parameters[:override_num_layers]
        ...
        if len([parameter for parameter in scalar_parameters]) != (override_num_layers or self.num_layers):
            weights = torch.tensor(self.weights, device=tensors[0].device)
            gamma = torch.tensor(self.gamma_value, device=tensors[0].device)
        else:
            weights = torch.cat([parameter for parameter in scalar_parameters])
            gamma = self.gamma
